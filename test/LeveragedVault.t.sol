...
contract MockPrimeBroker is IPrimeBroker {
    ...
    function getAvailableBorrow(address user, address assetAddr)
        external
        view
        override
        returns (uint256)
    ...
    function simulateApproveRequest(bytes32 requestId, address vault, uint256 approvedAmount)
        external
    {
    ...
    function simulateRejectRequest(bytes32 requestId, address vault, string calldata reason)
        external
    {
        require(leverageRequests[requestId].requestTimestamp != 0, "Invalid request");
        require(!leverageRequests[requestId].isProcessed, "Already processed");
    ...
    function processRequest(bytes32 requestId) external {
        ...
        // Call vault's approval handler
        LeveragedVaultImplementation(request.vault).handleBrokerApproval(
...
contract MockMorpho is IMorpho {
    ...
    function position(bytes32, address user)
        external
        view
        override
    ...
    function market(bytes32)
        ...
        pure
        override
        returns (uint128, uint128, uint128, uint128, uint256, uint128)
    {
