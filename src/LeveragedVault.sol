...
contract LeveragedVaultFactory is Ownable, ReentrancyGuard {
    ...
    function createVault(
        LeveragedVaultImplementation.VaultConfig memory config
    ) 
        external 
        nonReentrant 
        ...
        validVault(vaultId)
    {
        ...
        LeveragedVaultImplementation vaultContract = LeveragedVaultImplementation(vault.vaultAddress);
    ...
    function getVaultByAddress(address vaultAddress) external view returns (uint256 vaultId, VaultInfo memory vault) {
    ...
    function emergencyWithdrawFromVault(
        uint256 vaultId, 
        address token, 
        uint256 amount
